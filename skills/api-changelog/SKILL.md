...
Options:
- `--bootstrap` — emit exactly one target: latest stable 3.0.x → latest prerelease (use for first-run / PR bootstrap)
- `--output-dir <path>` — override default `lexicon/changelog/platform`
- `--surfaces <path>` — override default `skills/api-changelog/surfaces.platform.json`
...
`skills/api-changelog/surfaces.platform.json` is the authoritative list of SDK/client surfaces. The script also runs a **drift detector**: any `packages/` directory at the head tag that matches SDK-ish naming patterns but is not in the allow-list or `exclude_reason` emits a WARNING to stderr. Warnings are non-fatal — they flag new packages for a human to classify.
...
> ### Output format
>
> Write the surface H2 section using the format in `skills/api-changelog/SKILL.md → Output Format`. Keywords must be **exact symbol/RPC/type names** as they appear in the public API — grep-exact, backtick-wrapped. Every row needs a `Ref` column entry.
>
...
<!-- Auto-generated by api-changelog. Do not edit. Generated: YYYY-MM-DD -->
